...
window.addEventListener('load', function handleNavigationLoad () {
  ...
  if (navOpenEl) {
    ...
    //   https://github.com/twolfson/youtube-music-electron/issues/43
...
function handleLoad() {
    ...
    // observe some ui states -> send to ipc
  ...
  // var MutationObsçerver = this.win.MutationObserver || this.win.WebKitMutationObserver;
  ...
  debugger;
  var controls = document.getElementsByClassName("play-pause-button")[0];
  var observer = new MutationObserver(function handleChange(data) {
